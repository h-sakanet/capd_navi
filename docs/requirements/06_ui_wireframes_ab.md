...
> 移行メモ（2026-02-11）: 画面ごとの一次仕様ワイヤーは `docs/spec/20_screens/SCR-*.md` に移行済みです。  
> 本書は旧要求仕様系の参照用として残置しています。

## 1. 目的
実装前にUI仕様を固定し、画面解釈のズレを抑えるためにホーム/セッションのA案を確定します。  
ホームは「手技開始ハブ + 当日CAPDノート表示」に限定し、過去記録は別画面へ分離します。

本書は「見た目と配置」の仕様を定義します。  
画面遷移・操作副作用は `10_screen_transition_and_actions.md`、フォーム項目順は `11_form_contracts.md`、表示データの入出力は `12_ui_data_binding_matrix.md` を正本とします。
