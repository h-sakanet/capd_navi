...
| AT-REC-004 | 最終完了時必須判定 | `session_summary` 記録済み | 必須欠落のまま最終ステップ完了 | 完了拒否（`422 SESSION_SUMMARY_REQUIRED_MISSING`） |
...
| AT-REC-010 | summaryScope算出 | `session_summary` 保存済み | `payload.summaryScope` 未指定または不正値で保存後、最終ステップ完了 | サーバーが `completedAt` 基準で `first_of_day/last_of_day/both` を再計算して保存する |
| AT-REC-011 | summaryScope不正値の受理 | `session_summary` 保存 | `payload.summaryScope` に不正値を含めて `POST /sessions/{id}/records` | `422` にならず保存成功し、`summaryScope` は破棄され他の妥当項目は保存される |
...
| AT-UI-HOME-019 | 確認モード副作用なし | Home Aで手技設定済み | `•••`→確認→手順表示→ホームへ戻る | 記録保存・タイマー開始・通知ジョブ生成・スロット/セッション状態更新が発生しない |
...
| ID | 観点 | 前提 | 操作 | 期待結果 |
|---|---|---|---|---|
| AT-OPS-001 | 手動30秒テスト | 任意タイミング（通知設定変更時/OS更新後/通知不調時） | テスト通知実行 | 30秒テスト通知を手動で実施できる |
| AT-OPS-002 | テスト失敗時運用切替 | 手動30秒テスト失敗 | タイマー運用開始 | 当日を「iPhone補助なし（Mac主導）」として扱える |
