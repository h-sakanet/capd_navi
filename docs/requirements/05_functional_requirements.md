...
- FR-009A: 右上 `•••` メニューから「確認/編集」を開きます。確認モーダルは閲覧専用とし、ここからの「開始/再開」は手順表示のみ（確認モード）とします。確認モードでは記録保存、タイマー開始、通知ジョブ生成、アラーム設定、スロット/セッション状態更新、DB更新を行いません。
...
- FR-044D: `POST /sessions/{id}/records` で `payload.summaryScope` が未指定または不正値でも `422` で拒否せず、`summaryScope` 入力値のみ破棄して他の妥当な入力値を保存します。
