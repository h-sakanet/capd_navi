...
- 当日最初のタイマー運用前に30秒テスト通知を実施します。
- 30秒テストが失敗した日は iPhone補助を無効扱いとし、Mac主導運用へ切替します。
...
13. 最終ステップでセッション完了
14. セッション完了時に `session_summary` の必須項目を判定（最初完了セッション: 血圧/体重/脈拍/体温/出口部状態、最後完了セッション: 飲水量/尿量/排便回数）
