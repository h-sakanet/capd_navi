...
| ID | 観点 | 前提 | 操作 | 期待結果 |
|---|---|---|---|---|
| AT-CSV-001 | 正常取込 | 正常CSV（固定ヘッダー） + 画像 | 取込実行 | 成功しテンプレート登録 |
...
### 2.11 ストレージ管理（開発/検証専用）
| ID | 観点 | 前提 | 操作 | 期待結果 |
|---|---|---|---|---|
| AT-DEV-STORAGE-001 | 可視化 | `ENV=true` かつストレージにデータあり | 管理画面を表示 | localStorage/IndexedDB 一覧と詳細プレビューを確認できる |
| AT-DEV-STORAGE-002 | 任意削除 | 削除対象を選択済み | 削除を確定 | 指定した対象のみ削除される |
| AT-DEV-STORAGE-003 | 全削除 | ストレージにデータあり | `DELETE` 入力後に全削除実行 | localStorage + IndexedDB が空になる |
| AT-DEV-STORAGE-004 | 公開制御 | `ENV!=true` | 管理画面へアクセス | 404 になる |
