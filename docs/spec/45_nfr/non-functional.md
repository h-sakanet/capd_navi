...
- FR-092の対象: `index.json`（manifest）, `days/*.json`（DayBundle）, `photos/*`（JPEG）をバックアップ対象とします。
- 日次バックアップはローカル正本（IndexedDB）から生成したクラウド共有データの世代保持を指し、アプリ設定や一時キャッシュは対象外です。
...
## 7. 提供形態/プラットフォーム
- FR-103: iOSネイティブアプリ配布はv1対象外。
- FR-109: 提供形態は `Web本体 + Mac薄型ネイティブシェル（WKWebView）` を前提に運用。
- FR-110: MacネイティブシェルはOS通知連携を提供。
- FR-111: Macネイティブシェルは画面スリープ抑止を提供。
- FR-112: iPhoneはPWA通知対応時のみOS通知を利用。
- FR-113: スリープ抑止未対応/失敗時は注意表示と対処ガイダンスを表示。
- FR-114: Web UIコンポーネント基盤は `shadcn/ui`。
- FR-115: Web UIアイコンセットは `MynaUI Icons`（`@mynaui/icons-react`）。
