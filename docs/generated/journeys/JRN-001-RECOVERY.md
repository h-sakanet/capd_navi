# JRN-001-RECOVERY マップ

```mermaid
flowchart LR
  %% JRN-001-RECOVERY
  subgraph JRN["Journeys"]
    N34["JRN-001-RECOVERY 復旧（DB消失/クラウド欠損）"]
  end
  subgraph ACT["Actions"]
    N1["ACT-SYNC-001 自動同期（起動/復帰/完了）"]
  end
  subgraph SCR["Screens"]
    N35["SCR-SYNC-STATUS-001 同期状態表示"]
  end
  ...
  subgraph FR["Functional Requirements"]
    ...
    N32["- FR-087D: 全量再シード失敗時はローカルデータを不変のまま保持し、lastSyncStatus=failed と再試行導線を表示します。"]
    N33["- FR-088: 同期失敗時は直近同期状態（成功/失敗）を更新し、再試行導線を表示します。"]
  end
